        ...
        {items.map((item) =>
          onHome ? (
            ...
            <Link
              ...
              to="/"
              hash={item.id}
              // WORK manages its own scroll (it may need to
              // land on an open category grid, not just the
              // top of the section or the page) - so we let
              // WorkSection handle positioning itself instead
              // of the router jumping there first.
              hashScrollIntoView={item.id !== "work"}
              resetScroll={item.id !== "work"}
