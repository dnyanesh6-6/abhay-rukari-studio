...
/** Fast cinematic name reveal shown once before the hero. */
export function NameReveal({ onDone }: { onDone?: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        opacity: leaving ? 0 : 1,
        transform: leaving ? "translateY(-2%) scale(1.02)" : "none",
      }}
    >
      <div className="overflow-hidden px-6">
        <h1 className="font-display rise text-[clamp(2rem,9vw,6rem)] leading-[0.95] font-semibold">
          {identity.firstName} <span className="text-accent">{identity.lastName}</span>
        </h1>
      </div>
